...
@section('main')
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header d-flex justify-content-between align-items-center">
                                <h4 class="mb-0">{{ __('dashboard.mark_attendance') }}</h4>
                                ...
                                <div>
                                    ...
                                    <button type="button" class="btn btn-primary" data-toggle="modal"
                                        ...
                                        {{ __('dashboard.add_attendance') }}
                                    </button>
                                    <!-- Scan QR Code Button -->
                                    <a href="{{ route('employee.card.scan') }}" class="btn btn-success">
                                        <i class="fas fa-qrcode"></i>
                                        {{ __('dashboard.scan_qr') }}
                                    </a>
                                    ...
                                    <table class="table table-striped table-hover" id="tableExport" style="width:100%;">
                                        <thead>
                                            <tr>
                                                ...
                                                <th>{{ __('dashboard.overtime_status') }}</th>
            ...
            <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-header">
                        <h5 class="modal-title" id="addAttendanceModalLabel">{{ __('dashboard.add_attendance') }}</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>

                    <form action="{{ route('dashboard.employee.attendance.store') }}" id="addAttendanceModal"
                        method="POST">
                        @csrf
                        <div class="modal-body">
                            ...
                            <div class="form-row">
                                <div class="form-group col-md-6">
                                    <label for="employeeName">{{ __('dashboard.employee') }} <span
                                ...
                                <div class="form-group col-md-6">
                                    <label for="attendanceDate">{{ __('dashboard.date') }} <span
                                            class="text-danger">*</span></label>
                                    <input type="date" class="form-control @error('date') is-invalid @enderror"
                                        id="attendanceDate" name="date" value="{{ old('date') }}">
                            ...
                            <div class="form-row">
                                <div class="form-group col-md-6">
                                    <label for="clockIn">{{ __('dashboard.clock_in') }}</label>
                                    <input type="time" class="form-control @error('check_in') is-invalid @enderror"
                                        id="clockIn" name="clock_in" value="{{ old('check_in') }}">

                                </div>

                                <div class="form-group col-md-6">
                                    <label for="clockOut">{{ __('dashboard.clock_out') }}</label>
                                    <input type="time" class="form-control @error('check_out') is-invalid @enderror"
                                        id="clockOut" name="clock_out" value="{{ old('check_out') }}">

                                </div>
                            </div>

                            <div class="form-group">
                                <label for="status">{{ __('dashboard.status') }} <span
                                        class="text-danger">*</span></label>
                            ...
                            <div class="form-row">
                                <div class="form-group col-md-6">
                                    <label for="overtime_hours">{{ __('dashboard.overtime') }}
                        ...
                        <div class="modal-footer">
                            ...
                            <button type="submit" class="btn btn-primary">{{ __('dashboard.save_attendance') }}</button>
                        </div>
                    </form>
                    ...
                    <form id="editAttendanceForm">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="form-row">
                                ...
                                <div class="form-group col-md-6">
                                    <label for="editOvertimeStatus">{{ __('dashboard.overtime_status') }}</label>
                                    <select id="editOvertimeStatus" class="form-control" name="overtime_status">
    ...
    <script>
        ...
        $(document).on('click', '.edit-attendance-btn', function() {
            ...
            $('#editOvertimeStatus').val(attendance.overtime_status);
        ...
        // Helper function to safely display time in HH:MM
        function formatTime(time) {
            // If time is null or empty, show "-"
            ...
            $.ajax({
                ...
                `;
                        ...
                        data.forEach((att, index) => {
                            // Status badge
                            let statusBadge = '';
                            if (att.status === 'Present') statusBadge =
                                '<span class="badge badge-success">Present</span>';
                            else if (att.status === 'Absent') statusBadge =
                                '<span class="badge badge-danger">Absent</span>';
                            else statusBadge = '<span class="badge badge-warning">Leave</span>';

                            // Overtime status badge
                            let overtimeBadge = '-';

                            if (att.overtime_status === 'approved') {
                                overtimeBadge = '<span class="badge badge-success">Approved</span>';
                            } else if (att.overtime_status === 'cancelled') {
                                overtimeBadge = '<span class="badge badge-danger">Cancelled</span>';
                            } else if (att.overtime_status === 'pending') {
                                overtimeBadge = '<span class="badge badge-warning">Pending</span>';
                            }
                        ...
                        <tr>
                            <td>${index + 1}</td>
                            <td>${att.employee ? att.employee.first_name + ' ' + (att.employee.last_name ?? '') : 'N/A'}</td>
                            <td>${att.date ?? '-'}</td>
...
<td>${overtimeBadge}</td>
                            ...
                            <td>
                                <a href="#" class="text-secondary edit-attendance-btn" data-toggle="modal"
                                    ...
                                    data-overtime_status="${att.overtime_status ?? 'pending'}"
                                ...
                                <a href="#" class="text-danger delete-attendance-btn" data-toggle="modal"
                                    ...
                                    data-id="${att.id}"
                                    data-employee="${att.employee ? att.employee.first_name + ' ' + (att.employee.last_name ?? '') : 'N/A'}"
                                    data-date="${att.date ?? ''}">
