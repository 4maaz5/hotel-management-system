...
@php
    ...
    $attachedPenaltyIds = $cancelReason->penalties->pluck('id')->toArray();
@endphp
...
@section('content')
    <main class="u-white-bg bg-white p-3" style="border-radius:15px;">
        ...
        <form method="POST" action="{{ route('setup-sidebar.cancel_reason.update', $cancelReason->id) }}">
            @csrf
            @method('PUT')
            ...
            <div class="penalty-grid">
                <div class="row">
                    <div class="col-md-4">
                        ...
                        <input type="text" name="name" class="form-control" value="{{ $cancelReason->name }}"
                            required>
                    </div>
                </div>
                ...
                <div class="row u-mt-10">
                    <div class="col-md-4">
                        ...
                        <textarea name="description" class="form-control" rows="5">{{ $cancelReason->description }}</textarea>
                    </div>
                </div>
                    ...
                    <div class="col-md-4">
                        <div class="form-check form-switch custom-switch">
                            <input class="form-check-input" type="checkbox" name="is_active" id="is_active"
                                {{ $cancelReason->is_active ? 'checked' : '' }}>
                ...
                <table class="table table-bordered" id="penaltyTable">
                    ...
                    <tbody id="penaltyTableBody">
                        @foreach ($cancelReason->penalties as $penalty)
                            <tr data-penalty-id="{{ $penalty->id }}">
                                <td>
                                    <input type="hidden" name="penalties[]" value="{{ $penalty->id }}">
                                    {{ $penalty->name }}
                                </td>
                                <td>
                                    <div class="form-check form-switch custom-switch">
                                        <input class="form-check-input" type="checkbox"
                                            name="auto_apply_{{ $penalty->id }}" value="1"
                                            {{ $penalty->pivot->auto_apply ? 'checked' : '' }}>
                                    </div>
                                </td>
                                <td>
                ...
                <div class="u-mt-15">{{ __('dashboard.total') }}: <span
                        id="penaltyCount">{{ $cancelReason->penalties->count() }}</span></div>
            </div>
...
@push('scripts')
    <script>
        ...
        const attachedPenaltyIds = @json($attachedPenaltyIds);
