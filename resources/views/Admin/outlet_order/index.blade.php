...
@section('content')
    <main class="u-white-bg bg-white p-3" style="border-radius:15px;">
        ...
        <div class="page-header">
            <div>
                <h2 class="page-header__title">{{ __('dashboard.orders_list') }}</h2>
            </div>
        </div>
                    ...
                    <tbody>
                        @forelse($orders ?? [] as $index => $order)
                            <tr>
                                ...
                                <td>{{ number_format($order->total ?? 0, 2) }} SAR</td>
                                <td>{{ $order->guest->first_name ?? '-' }} {{ $order->guest->last_name ?? '' }}</td>
                                <td>{{ $order->outlet->name ?? '-' }}</td>
                                <td>{{ $order->created_at ? $order->created_at->format('Y-m-d H:i') : '-' }}</td>
                                <td>{{ $order->reservation->reservation_number ?? '-' }}</td>
