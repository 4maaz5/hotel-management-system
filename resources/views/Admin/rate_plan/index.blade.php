...
@section('content')
    <main class="u-white-bg bg-white p-3" style="border-radius:15px;">
        ...
        <div class="page-header">
            <div>
                <h2 class="page-header__title">{{ __('dashboard.rate_plans') }}</h2>
                <div class="page-header__subtitle">{{ __('dashboard.view_and_manage_rate_plans') }}</div>
            </div>
            <div class="n-table__top-btns">
                ...
                <div>
                    @can('rate_plan.add')
                        <a href="{{ route('setup-sidebar.rate_plan.create') }}" class="n-button n-button--green"
                            style="text-decoration:none;" tabindex="0">
                            {{ __('dashboard.new_rate_plan') }}
                        </a>
                    @endcan
        ...
        <!-- Filter Form -->
        <form method="GET" action="{{ route('setup-sidebar.rate_plan.index') }}">
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="row g-3">
                            ...
                            <!-- Meals Dropdown -->
                            <div class="col-lg-3 col-md-4">
                                <label class="form-label">{{ __('dashboard.meals') }}</label>
                                <select name="meal" class="form-select">
                                    <option value="">{{ __('dashboard.all') }}</option>

                                    @foreach ($allMeals as $meal)
                                        <option value="{{ $meal }}"
                                            {{ request('meal') == $meal ? 'selected' : '' }}>
                                            {{ ucfirst($meal) }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-lg-2 col-md-4 d-flex align-items-end">
                                ...
                                <a href="{{ route('setup-sidebar.rate_plan.index') }}" class="btn btn-outline-secondary">
                                    {{ __('dashboard.reset') }}
                                </a>
                            </div>
                ...
                <tbody>
                    @forelse ($ratePlans as $ratePlan)
                        <tr>
                            ...
                            <td>{{ $ratePlan->name }}</td>
                            <td>
                                @if ($ratePlan->meals->isNotEmpty())
                                    {{ $ratePlan->meals->pluck('meal_name')->join(', ') }}
                                @else
                            ...
                            <td>
                                @can('rate_plan.view')
                                    <!-- View button -->
                                    <a href="{{ route('setup-sidebar.rate_plan.view', $ratePlan->id) }}"
                                ...
                                @can('rate_plan.edit')
                                    <!-- Edit button -->
                                    <a href="{{ route('setup-sidebar.rate_plan.edit', $ratePlan->id) }}"
                                ...
                                @can('rate_plan.delete')
                                    <button class="btn btn-sm btn-danger" data-bs-toggle="modal"
                                        data-bs-target="#deleteCustomRateModal{{ $ratePlan->id }}">
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="text-center text-muted">
                                {{ __('dashboard.no_rate_plans_found') }}
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    @foreach ($ratePlans as $ratePlan)
        <div class="modal fade" id="deleteCustomRateModal{{ $ratePlan->id }}" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">
                            {{ __('dashboard.delete_rate_plan') }} – {{ $ratePlan->name }}
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                    </div>
                    <div class="modal-body text-center">
                        <p>{{ __('dashboard.delete_rate_plan_confirmation') }}</p>
                    </div>
                    <div class="modal-footer justify-content-center">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">
                            {{ __('dashboard.cancel') }}
                        </button>
                        <form action="{{ route('setup-sidebar.rate_plan.delete', $ratePlan->id) }}" method="POST">
                            @csrf
                            @method('DELETE')
