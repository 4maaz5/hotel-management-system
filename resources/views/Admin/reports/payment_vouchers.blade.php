...
@section('title', __('dashboard.payments_report'))

@section('report_content')
    <div class="card shadow-sm">
        <div class="card-header">
            <h5 class="mb-0">{{ __('dashboard.payment_vouchers') }}</h5>
        </div>
                    ...
                    <tbody>
                        @forelse($vouchers as $voucher)
                            <tr>
                                ...
                                <td>{{ $voucher->paymentMethod->paymentMethod->name ?? '-' }}</td>
                                <td>{{ $voucher->vendor_name ?? '-' }}</td>
                                <td>{{ $voucher->purpose ?? '-' }}</td>
                                <td class="text-end text-danger">SAR {{ number_format($voucher->amount, 2) }}</td>
                            </tr>
                        @empty
                            <tr>
...
<div class="english-text">
    <h4 class="text-center mb-3">{{ __('dashboard.payments_report') }}</h4>
...
<div class="arabic-text">
    <h4 class="text-center mb-3">{{ __('dashboard.payment_vouchers_report_ar') }}</h4>
