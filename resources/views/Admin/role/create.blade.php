...
@section('content')
    <div class="bg-white p-3" style="border-radius:10px;">
        ...
        <div class="page-header">
            <div>
                <h2 class="page-header__title">{{ __('dashboard.new_role') }}</h2>
                <div class="page-header__subtitle">{{ __('dashboard.create_a_new_role_for_the_system') }}</div>
            </div>
        </div>

        <form method="post" action="{{ route('setup-sidebar.property-role.store') }}" novalidate id="form">
            @csrf
            <div class="row mb-4">
                <div class="col-md-6">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="description" class="form-control max-height-250" rows="4"
            ...
            <div class="radio-collapse u-mb-24">
                <label class="radio-collapse__header">
                    <div class="radio-collapse__radio">
                        <input type="radio" name="access_type" id="full-access" value="full" class="k-radio" checked>
                    </div>
                    <div>
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const allSelectAlls = document.querySelectorAll('.select-all-permissions');
            allSelectAlls.forEach(selectAll => {
                selectAll.addEventListener('change', function() {
                    const panel = this.closest('.permission-panel'); // only inside the panel
                    const checkboxes = panel.querySelectorAll('.permission-checkbox');
