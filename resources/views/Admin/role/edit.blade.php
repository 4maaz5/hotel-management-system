...
@section('content')
   <div class="bg-white p-3" style="border-radius:10px;">
        ...
        <div class="page-header">
            <div>
                <h2 class="page-header__title">{{ __('dashboard.edit_role') }}</h2>
                <div class="page-header__subtitle">{{ __('dashboard.edit_role_for_the_system') }}</div>
            </div>
        </div>

        <form method="POST" action="{{ route('setup-sidebar.property-role.update', $role->id) }}" novalidate id="form">
            @csrf
            @method('PUT')
                ...
                <div class="col-md-6">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="name" class="form-control"
                            placeholder="{{ __('dashboard.enter_role_name') }}" value="{{ old('name', $role->name) }}">
                        @error('name')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
                    </div>
