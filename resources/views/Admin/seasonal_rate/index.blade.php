...
@section('content')
    <main class="u-white-bg bg-white p-3" style="border-radius:15px;">
        ...
        <div class="page-header">
            <div>
                <h2 class="page-header__title">{{ __('dashboard.seasonal_rates') }}</h2>
                <div class="page-header__subtitle">{{ __('dashboard.view_and_manage_seasonal_rates') }}</div>
            </div>
            <div class="n-table__top-btns">
                ...
                <div>
                    @can('seasonal_rate.add')
                        <a href="{{ route('setup-sidebar.seasonal_rate.create') }}" class="n-button n-button--green"
                            style="text-decoration:none;" tabindex="0">
                            {{ __('dashboard.add_new_seasonal') }}
                        </a>
                    @endcan
        ...
        <form method="GET" action="{{ route('setup-sidebar.seasonal_rate.index') }}"
            class="card card-body mb-4 filter-form__container">
            <div class="row g-3">
                ...
                <div class="col-xl-4 col-md-6">
                    ...
                    <div class="row">
                        <div class="col-6">
                            <input type="date" name="end_from" value="{{ request('end_from') }}" class="form-control">
                        </div>
                        <div class="col-6">
                ...
                <div class="col-12 text-end">
                    ...
                    <a href="{{ route('setup-sidebar.seasonal_rate.index') }}" class="btn btn-secondary">
                        {{ __('dashboard.reset') }}
                    </a>
                </div>
                ...
                <tbody>
                    @forelse($seasons as $season)
                        <tr>
                            ...
                            <td>
                                {{ $season->start_date->format(system_date_format()) }}
                            </td>

                            <td>
                                {{ $season->end_date->format(system_date_format()) }}
                            </td>

                            <td>
                                @can('seasonal_rate.view')
                                    <!-- View -->
                                    <a href="{{ route('setup-sidebar.seasonal_rate.view', $season->id) }}"
                                        class="btn btn-sm btn-primary me-1">
                                        <i class="bi bi-eye"></i>
                                    </a>
                                @endcan

                                @can('seasonal_rate.edit')
                                    <!-- Edit -->
                                    <a href="{{ route('setup-sidebar.seasonal_rate.edit', $season->id) }}"
                                        class="btn btn-sm btn-warning me-1">
                                        <i class="bi bi-pencil-square"></i>
                                    </a>
                                @endcan

                                @can('seasonal_rate.delete')
                                    <button class="btn btn-sm btn-danger" data-bs-toggle="modal"
                                        data-bs-target="#deleteCustomRateModal{{ $season->id }}">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="text-center text-muted">
                                {{ __('dashboard.no_seasonal_rates_found') }}
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    @foreach ($seasons as $season)
        <div class="modal fade" id="deleteCustomRateModal{{ $season->id }}" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">
                            {{ __('dashboard.delete_custom_rate') }} – {{ $season->name }}
                        </h5>
                    ...
                    <div class="modal-footer justify-content-center">
                        ...
                        <form action="{{ route('setup-sidebar.seasonal_rate.delete', $season->id) }}" method="POST">
                            @csrf
                            @method('DELETE')
